...
const ShareCard = () => {
  ...
  const addUserToSocketDB = () => {
    // Check if socket exists before adding event listener
    if (!userDetails.socket) return;
  ...
  useEffect(() => {
    // Set user ID immediately when component mounts
    if (userDetails && userDetails.userId) {
      console.log("Setting userId directly:", userDetails.userId);
      setuserId(userDetails.userId);
    }
    ...
    if (userDetails.socket && userDetails.socket.connected) {
      console.log("Socket already connected, setting up user in DB");
      setuserId(userDetails.userId);
      userDetails.socket.emit("details", {
        socketId: userDetails.socket.id,
        uniqueId: userDetails.userId,
      });
    } else if (userDetails.socket) {
    ...
    workerRef.current?.addEventListener("message", (event: any) => {
      if (event.data?.progress) {
        ...
        // Set the actual blob data for download
        console.log("Received blob data for download, size:", event.data.blob.size);
        setdownloadFile(event.data?.blob);
        // Reset progress on the receiver's side
        setfileDownloadProgress(100); // Set to 100% when complete
        setTimeout(() => {
          setfileDownloadProgress(0);
        }, 1000);
        setfileReceiving(false);
        toast.success("File received and ready for download!");
      }
    });
  ...
  function handleReceivingData(data: any) {
    if (data.info) {
      // File info received, prepare worker
      ...
      console.log("File info received:", data.fileName, data.fileSize, data.fileType);
    } else if (data.done) {
      // File transfer complete, tell worker to create download blob
      console.log("File transfer complete, creating download");
      workerRef.current?.postMessage("download");
    } else {
      // Receiving chunk data, pass to worker
      if (!downloadFile) {
        // Only set this once to indicate we're in downloading mode
        setdownloadFile({});
      }
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <div className="flex flex-row justify-left items-center space-x-2">
                  <div className="flex border rounded-md px-3 py-2 text-sm h-10 w-full bg-muted">
                    {userId || userDetails?.userId ? (userId || userDetails?.userId) : "-"}
                  </div>
                  <Button
                    variant="outline"
                    type="button"
                    className="p-4"
                    onClick={() => CopyToClipboard(userId || userDetails?.userId)}
